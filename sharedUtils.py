...
def scraper(driver, start_month, start_day, start_year, end_month, end_day, end_year, state='oh'):
    def textGrabber(variable):
        ...
        radio_button = driver.find_element(By.CSS_SELECTOR, f'input[name="variable"][type="radio"][value="{variable}"]')
        radio_button.click()
    ...
    # Combine DataFrames
    final_df = pd.concat([df1[['cd', 'DD', 'Normal', 'Departure', 'Percent']],
                          df2[['DD', 'Normal', 'Departure', 'Percent']]], axis=1)
    final_df.columns = ['cd', 'W_DD', 'W_Normal', 'W_Departure', 'W_Percent', 'C_DD', 'C_Normal', 'C_Departure',
                        'C_Percent']
